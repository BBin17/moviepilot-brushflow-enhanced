...
最后实机验证：2026-09-05，MoviePilot 已更新到 9.0.2，咖啡、馒头和憨憨三个任务及历史配置保留并继续运行；9.0.2 健康卡、精确删种原因和右上角单一新建入口已实机核对。
...
5. 整页刷新 MoviePilot，再从侧栏重新进入 **站点刷流**。远程前端组件不会随插件后台更新自动卸载；不刷新可能继续显示旧版界面，即使插件卡片已经是新版本。
6. 确认任务列表、任务数量和策略概览仍在；不需要重新创建任务。

本次 9.0.2 的核验结果：咖啡、馒头刷流、憨憨均为运行中；咖啡容量严重超限时能显示具体额度拦截原因；已有任务时不再显示重复的大型新建任务卡，右上角新建入口保留。
...
- 插件卡片已是新版本，但站点刷流界面仍像旧版：先整页刷新浏览器并从侧栏重新进入插件页。这是浏览器仍持有升级前远程组件实例，不要重复安装或重置插件。
