...
class TorrentObservation:
    ...
    demand_confirmations: Optional[int] = None
    low_value_confirmations: Optional[int] = None
    ...
    def from_mapping(cls, data: Mapping[str, Any]) -> "TorrentObservation":
        ...
        return cls(
            ...
            demand_confirmations=max(int(_number(data["demand_confirmations"])), 0) if data.get("demand_confirmations") is not None else None,
            low_value_confirmations=max(int(_number(data["low_value_confirmations"])), 0) if data.get("low_value_confirmations") is not None else None,
...
def deletion_budget(policy: SmartPolicy, *, active_count: int, capacity: float, deleted_count=0, deleted_bytes=0.0) -> dict:
    """Shared rolling-window limits for normal and invalid-tracker cleanup.

    A zero percentage disables that individual quota, preserving legacy custom
    settings. A positive explicit GB limit still applies; manual policy supplies
    an unconditional 10 / 25% ceiling separately.
    """
    count_cap = (max(1, math.floor((active_count + max(deleted_count, 0)) * policy.max_delete_percent_day / 100))
                 if policy.max_delete_percent_day > 0 else active_count + max(deleted_count, 0))
    run_caps = [cap for cap in (capacity * policy.max_delete_capacity_percent_run / 100, policy.max_delete_gb_per_run * 1024**3) if cap > 0]
    day_caps = [cap for cap in (capacity * policy.max_delete_capacity_percent_day / 100, policy.max_delete_gb_per_day * 1024**3) if cap > 0]
    day_bytes = min(day_caps) if day_caps else 0.0
    return {"run_count_cap": max(int(policy.max_delete_per_run), 0), "daily_count_cap": count_cap,
            "remaining_daily_count": max(count_cap - max(deleted_count, 0), 0),
            "run_byte_cap": min(run_caps) if run_caps else 0.0, "daily_byte_cap": day_bytes,
            "remaining_daily_bytes": max(day_bytes - max(deleted_bytes, 0.0), 0)}
...
def _demand_confirmations(
    ...
    if observation.demand_confirmations is not None:
...
def _confirmation_state(
    ...
    if observation.low_value_confirmations is not None:
...
def select_deletions(
    ...
    *,
    ...
    budget = deletion_budget(policy, active_count=len(normalized), capacity=disk_limit or current_size,
                             deleted_count=deleted_today, deleted_bytes=deleted_today_bytes)
    daily_count_cap = budget["daily_count_cap"]
    remaining_count = budget["remaining_daily_count"]
    run_count_cap = budget["run_count_cap"]
