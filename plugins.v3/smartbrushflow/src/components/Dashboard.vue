...
async function loadStatus() {
  ...
  loading.value = true
  try {
    status.value = unwrapResponse(await props.api.get('plugin/SmartBrushFlow/status')) || status.value
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="smartbrushflow-dashboard">
    <div class="smartbrushflow-dashboard__metrics">
    ...
    <VDivider />
    <div class="smartbrushflow-dashboard__tasks">
      ...
      <div v-if="!runningTasks.length" class="smartbrushflow-dashboard__empty">
...
.smartbrushflow-dashboard__metrics {
  display: grid;
  grid-template-columns: repeat(4, minmax(0, 1fr));
  gap: 12px;
}

.smartbrushflow-dashboard__metrics > div {
  display: flex;
  flex-direction: column;
  gap: 3px;
  min-inline-size: 0;
...
.smartbrushflow-dashboard__metrics span,
.smartbrushflow-dashboard__tasks span {
  color: rgba(var(--v-theme-on-surface), var(--v-medium-emphasis-opacity));
  font-size: 0.78rem;
}

.smartbrushflow-dashboard__metrics strong {
  font-size: 1.05rem;
  overflow-wrap: anywhere;
}

.smartbrushflow-dashboard__tasks {
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.smartbrushflow-dashboard__tasks > div {
  display: grid;
...
.smartbrushflow-dashboard__tasks > div > div {
  display: flex;
  flex-direction: column;
  min-inline-size: 0;
}

.smartbrushflow-dashboard__tasks strong {
  overflow-wrap: anywhere;
}

.smartbrushflow-dashboard__empty {
