...
from .downloaders import DownloaderAdapter, DownloaderUnavailable
...
class SmartBrushFlow(_PluginBase):
    ...
    def init_plugin(self, config: dict = None) -> None:
        ...
        self._task_service = TaskService(self._repository, locks=self._task_locks, runtime=self._runtime, runtime_lock=self._runtime_lock)
        for task_id in self._task_configs:
            self._task_service.recover_interrupted(task_id)
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "path": "/tasks/{task_id}/cleanup/preview",
            ...
            {
                "path": "/tasks/{task_id}/operations/{operation_id}",
                "endpoint": self.get_operation,
                "methods": ["GET"],
                "auth": "bear",
                "summary": "读取操作进度与逐项结果，不触发下载器请求",
            },
            {
                "path": "/signin/run",
    ...
    def _update_task_locked(self, task_id: str, payload: TaskConfigV9) -> schemas.Response:
        """更新任务配置并保持原任务 ID 与历史数据关联"""
        if task_id not in self._task_configs:
            return schemas.Response(success=False, message="刷流任务不存在")
        previous_document = self._task_documents[task_id]
        if payload.revision != previous_document.revision:
            return self._operation_error(OperationError("revision_conflict", "任务配置已被更新，请刷新后重试"))
        document = payload.model_copy(deep=True)
        document.id = task_id
    ...
    def delete_task(self, task_id: str) -> schemas.Response:
        try:
            with self._get_task_service().guard(task_id):
                return self._delete_task_locked(task_id)
        except OperationError as error:
            return self._operation_error(error)

    def _delete_task_locked(self, task_id: str) -> schemas.Response:
        """删除没有活跃种子的任务及其独立历史数据"""
    ...
    def _get_task_service(self) -> TaskService:
        if not getattr(self, "_task_service", None):
            repository = getattr(self, "_repository", None) or TaskRepository(self)
            self._task_service = TaskService(repository, locks=self._task_locks, runtime=self._runtime, runtime_lock=self._runtime_lock)
        return self._task_service

    @staticmethod
    def _operation_error(error: OperationError) -> schemas.Response:
        return schemas.Response(success=False, message=str(error), data={"code": error.code})

    def _deletion_service(self, task_id: str) -> DeletionService:
        task = self._task_configs[task_id]
        service = DownloaderHelper().get_service(name=task.downloader)
        if not service or not service.instance:
            raise DownloaderUnavailable("下载器不可用")
        return DeletionService(
            repository=self._repository, operations=self._get_task_service(), document=self._task_documents[task_id],
            managed_tag=task.brush_tag,
            adapter=DownloaderAdapter(service.instance, identify=self.__get_hash,
                                      normalize=lambda torrent: self.__get_torrent_info(torrent, service=service)),
        )
    ...
    def preview_cleanup(self, task_id: str, payload: CleanupPreviewPayload) -> schemas.Response:
        if task_id not in self._task_documents:
            return schemas.Response(success=False, message="刷流任务不存在")
        try:
            with self._get_task_service().guard(task_id), self._task_scope(task_id):
                if payload.revision != self._task_documents[task_id].revision:
                    raise OperationError("revision_conflict", "任务配置已改变，请刷新后预览")
                result = self._deletion_service(task_id).preview(relax_limits=payload.relax_limits)
            return schemas.Response(success=True, data=result)
        except OperationError as error:
            return self._operation_error(error)
        except DownloaderUnavailable:
            return schemas.Response(success=False, message="下载器不可用，未生成清理预览", data={"code": "downloader_unavailable"})

    def get_operation(self, task_id: str, operation_id: str) -> schemas.Response:
        if task_id not in self._task_documents:
            return schemas.Response(success=False, message="刷流任务不存在")
        row = self._get_task_service().get(task_id, operation_id)
        if row is None:
            return schemas.Response(success=False, message="操作不存在", data={"code": "operation_missing"})
        return schemas.Response(success=True, data={**row, "server_time": time.time()})

    def run_task_action(self, task_id: str, action: str, payload: Optional[TaskActionPayload] = None) -> schemas.Response:
        """Unified action API; a legacy cleanup call cannot bypass preview."""
        if task_id not in self._task_documents:
            return schemas.Response(success=False, message="刷流任务不存在")
        if action == "run_selection":
            return self._submit_task_operation(task_id, "brush", payload=payload)
        if action == "run_check":
            return self._submit_task_operation(task_id, "check", payload=payload)
        if action == "force_cleanup":
            if not payload or not payload.preview_id or not payload.request_id or not payload.confirm:
                return self._operation_error(OperationError("needs_preview", "请先预览并确认本轮清理名单"))
            return self._submit_task_operation(task_id, "cleanup", payload=payload)
        try:
            with self._get_task_service().guard(task_id):
                return self._apply_task_action(task_id, action)
        except OperationError as error:
            return self._operation_error(error)

    def _apply_task_action(self, task_id: str, action: str) -> schemas.Response:
        """Called only while holding the task mutation lock."""
        task = self._task_configs.get(task_id)
    ...
    def _submit_task_operation(
        self, task_id: str, operation: str, payload: Optional[TaskActionPayload] = None, *, background: bool = True,
    ) -> schemas.Response:
        task = self._task_configs.get(task_id)
        if not task:
            return schemas.Response(success=False, message="刷流任务不存在")
        payload = payload or TaskActionPayload()
        if operation not in {"brush", "check", "cleanup"}:
            return self._operation_error(OperationError("needs_preview", "请使用预览清理入口"))
        def validate():
            if not self.get_state() or not self._task_configs[task_id].enabled:
                raise OperationError("task_disabled", "插件或任务未启用")
            if operation == "cleanup":
                with self._task_scope(task_id):
                    self._deletion_service(task_id).validate_preview(payload)
        try:
            row = self._get_task_service().submit(
                task_id, operation,
                lambda operation_id: self._execute_task_operation(task_id, operation_id, operation, payload),
                dispatch=(lambda worker: ThreadHelper().submit(worker)) if background else None,
                request_id=payload.request_id,
                request_fingerprint=f"{payload.preview_id}:{payload.revision}:{payload.relax_limits}:{payload.confirm}",
                validate=validate,
            )
            return schemas.Response(success=row["state"] != "failed", message="任务已提交", data=row)
        except OperationError as error:
            return self._operation_error(error)
        except DownloaderUnavailable:
            return schemas.Response(success=False, message="下载器不可用，未提交清理", data={"code": "downloader_unavailable"})

    def _execute_task_operation(self, task_id: str, operation_id: str, operation: str, payload: TaskActionPayload) -> dict:
        report = self._new_run_report(operation)
        ...
        try:
            with self._task_scope(task_id):
                ...
                if not self.get_state() or not task.enabled:
                    raise OperationError("task_disabled", "插件或任务已暂停，本次未执行")
                if operation == "brush":
                    with self._brush_lock:
                        self._run_brush(task, report)
                elif operation == "check":
                    self._run_check(task, report, operation_id=operation_id)
                else:
                    cleanup = self._deletion_service(task_id)
                    preview = cleanup.validate_preview(payload)
                    result = cleanup.execute(operation_id, preview["items"], relax_limits=preview["relax_limits"], manual=True)
                    report.update(result)
                    report["result"] = result["state"]
                    self._recalculate_statistics(task_id)
                failed = report.get("result") in {"downloader_unavailable", "site_missing", "downloader_error", "failed"}
                report["success"] = not failed and (result.get("state") or report.get("operation_state")) not in {"partial", "pending_confirmation", "interrupted"}
                if not result:
                    result = {"state": "failed" if failed else report.get("operation_state", "completed"),
                              "phase": "检查完成" if operation == "check" else "选种完成",
                              "message": report.get("deletion_message") or report.get("result") or "操作完成"}
        except OperationError as error:
            report.update({"success": False, "error": str(error), "error_code": error.code})
            result = {"state": "failed", "phase": "操作未执行", "error": str(error), "error_code": error.code}
        except Exception:
            report.update({"success": False, "error": "操作失败，请检查下载器连接与运行诊断"})
            # Let the operation journal preserve potentially submitted items.
            raise
        finally:
    ...
    def _task_summary(
        ...
        cleanup_progress = self._get_task_service().latest(task_id, "cleanup")
        if (
            cleanup_progress
            and cleanup_progress.get("state") == "completed"
            and cleanup_progress.get("display_until") is not None
    ...
    def _build_strategy_status(self, task_id: str) -> Dict[str, Any]:
        ...
        if not isinstance(audit, list):
            audit = []
        current_plan = self._get_task_data(task_id, "current_deletion_plan") or {}
        ...
        actual_freed_24h = sum(
            ...
            and row.get("status") == "confirmed_removed"
            and bool(row.get("delete_data"))
        )
        ...
        recovery_state = self._get_task_data(task_id, "capacity_recovery") or {}
        recovery_active = bool(
            capacity
            and recovery_state.get("active", current_size >= capacity * task.smart_capacity_trigger_percent / 100)
            and current_size > (capacity_target or current_size)
        )
        torrent_sizes = {
        ...
        eligible_count = 0
        eligible_bytes = 0.0
        evaluated_rows = deletion_plan.get("evaluated") or (latest_deletion or {}).get("evaluated", [])
        for row in evaluated_rows:
            ...
            for code in row.get("reason_codes", row.get("reasons", [])) or []:
        ...
        deletion_reason_codes = list(deletion_plan.get("reason_codes") or [])
        planned_items = deletion_plan.get("items") or deletion_plan.get("selected") or []
        planned_count = len(planned_items)
        planned_bytes = (
            sum(float(row.get("size") or 0) for row in planned_items if row.get("delete_data"))
            if planned_items
            else float(deletion_plan.get("estimated_freed_bytes") or 0)
        )
        if not task.smart_enabled:
            readiness = {
                "state": "disabled",
        ...
        current_candidate_rows = [
            {
                ...
                "title": row.get("title") or torrents.get(str(row.get("hash") or ""), {}).get("title"),
                "size": float(row.get("size") or torrent_sizes.get(str(row.get("hash") or ""), 0)),
                "score": float(row.get("score") or 0),
                "reasons": list(row.get("reason_codes", row.get("reasons", [])) or []),
                "contributions": dict(row.get("contributions") or {}),
                "selected": any(item.get("hash") == row.get("hash") for item in planned_items),
            }
            for row in evaluated_rows
            if row.get("action") == "candidate"
        ]
    ...
    def brush(self, task_id: Optional[str] = None) -> None:
        """Scheduled and manual selection share the task operation lifecycle."""
        task = self._get_task_config(task_id)
        if task and self.get_state() and task.enabled:
            self._submit_task_operation(task.id, "brush", background=False)
    ...
    def check(self, task_id: Optional[str] = None, wait_for_lock: bool = False, force_cleanup: bool = False) -> None:
        """Scheduled checks cannot invoke an unpreviewed manual cleanup."""
        if force_cleanup:
            raise OperationError("needs_preview", "手动清理必须经过预览与确认")
        task = self._get_task_config(task_id)
        if task and self.get_state() and task.enabled:
            self._submit_task_operation(task.id, "check", background=False)
    ...
    def _run_check(self, task: BrushTaskConfig, report: dict, operation_id: Optional[str] = None) -> None:
        """Synchronize, sample, plan and execute through the same guarded service."""
        if not operation_id:
            raise OperationError("operation_required", "种子检查必须由统一任务服务执行")
        if not self._validate_task_reference(task) or not self.downloader:
            report["result"] = "downloader_unavailable"
            return
        downloader = self.downloader
        seeding_torrents, error = downloader.get_torrents()
        if error or seeding_torrents is None:
            raise DownloaderUnavailable("连接下载器出错")
        cleanup = self._deletion_service(task.id)
        cleanup.reconcile()
        ...
        report["download_health"] = self._build_download_health_summary(torrent_tasks)
        self.__update_undeleted_torrents_missing_in_downloader(torrent_tasks, check_hashes, seeding_torrents)
        self._save_current_task_data("torrents", torrent_tasks)
        history_before = self._current_task_data("smart_history", [])
        snapshot = {self.__get_hash(torrent): self.__get_torrent_info(torrent) for torrent in check_torrents}
        plan = cleanup.sample_and_plan(snapshot=snapshot)
        if task.smart_enabled:
            self._update_smart_strategy_state(task, plan["observations"], history_before,
                                              plan["selection"], plan["learning"], torrent_tasks)
            # Observation gating may have changed after the safety audit.
            cleanup = self._deletion_service(task.id)
        regular = plan["items"]
        invalid_items, working_domains = [], set()
        if task.invalid_seed_cleanup_enabled:
            ...
            invalid_hashes, invalid_reasons = self.__plan_invalid_seed_deletions(
                check_torrents, torrent_tasks, tracker_states, working_domains,
            )
            ...
            for key in invalid_hashes:
                ...
                invalid_items.append({
                    "hash": key, "title": torrent_tasks[key].get("title"),
                    "size": float(observation.get("total_size") or 0), "uploaded": float(observation.get("uploaded") or 0),
                    "kind": "invalid_tracker", "delete_data": False, "state": "pending",
                    "reason_codes": ["explicit_invalid_tracker_error"], "reason": invalid_reasons.get(key),
                })
        invalid_keys = {row["hash"] for row in invalid_items}
        items = invalid_items + [row for row in regular if row["hash"] not in invalid_keys]
        def validate_invalid(torrent_hash):
            current, current_error = downloader.get_torrents()
            if current_error or current is None:
                return False
            torrent = next((row for row in current if self.__get_hash(row) == torrent_hash), None)
            if torrent is None:
                return False
            return detect_invalid_seed(self.__get_tracker_items(torrent), working_domains=working_domains).invalid
        if items:
            result = cleanup.execute(operation_id, items, invalid_revalidator=validate_invalid)
            report.update(result)
        ...
        self._recalculate_statistics(task.id)
        report.update({
            "result": result["state"], "deleted_count": result["deleted_count"],
            "planned_delete_count": len(items), "selected_delete_count": len(items),
            "active_count": sum(not row.get("deleted") for row in torrent_tasks.values()),
            "invalid_seed_plan_count": len(invalid_items),
        })
        summary = self._build_strategy_status(task.id).get("ui_summary", {}).get("deletion", {})
        report.update({"deletion_state": summary.get("state"), "deletion_message": summary.get("message"),
                       "deletion_candidate_count": summary.get("candidate_count", 0),
                       "protected_count": summary.get("protected_count", 0), "protected_bytes": summary.get("protected_bytes", 0)})
    ...
    def __get_torrent_info(self, torrent: Any, service: Any = None) -> dict:
        ...
        if service and DownloaderHelper().is_downloader("qbittorrent", service=service):
            ...
            # Completion wall time includes pauses; only actual reported seeding
            # duration is admissible evidence for a site's minimum seed time.
            ...
            upload_speed = torrent.get("upspeed") if torrent.get("upspeed") is not None else torrent.get("upload_speed")
            ...
            seeding_time = getattr(torrent, "seconds_seeding", None)
        ...
        avg_upspeed = int((uploaded or 0) / dltime) if dltime else uploaded or 0
        return {
            "hash": torrent_id,
            "title": title,
