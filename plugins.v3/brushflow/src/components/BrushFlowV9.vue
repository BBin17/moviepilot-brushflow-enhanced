  ...
  <div class="bf9">
    ...
    <template v-if="selected">
      ...
      <VWindow v-model="tab" :touch="false">
        ...
        <VWindowItem value="history"><section class="panel mt-4"><h3>最近运行</h3><div class="event-list"><article v-for="run in runs" :key="run.id"><VIcon :icon="run.success===false?'mdi-alert-circle':'mdi-check-circle'" :color="run.success===false?'error':'success'"/><div><strong>{{ run.kind==='brush'?'选种刷新':'种子检查' }}</strong><span>{{ formatDateTime(run.started_at) }} · {{ run.kind==='brush'?`新增 ${run.added_count||0}，过滤 ${run.filtered_count||0}`:`活跃 ${run.active_count||0}，候选 ${run.deletion_candidate_count||0}，删除 ${run.deleted_count||0}` }}</span><small v-if="run.deletion_message" class="reason">{{ run.deletion_message }}</small><small v-if="run.error" class="error">{{ run.error }}</small></div></article><div v-if="!runs.length" class="bf9__empty small">暂无运行记录</div></div></section></VWindowItem>
      </VWindow>
    </template>
