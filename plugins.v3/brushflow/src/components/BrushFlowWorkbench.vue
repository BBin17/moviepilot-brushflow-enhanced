          ...
          <VCard class="brushflow-settings-menu" title="全局设置">
            <VCardText class="settings-menu__body">
              ...
              <VSwitch
                ...
                inset
              />
              <template v-if="settingsDraft.global_proxy_delete">
                <VTextField
                  v-model.number="settingsDraft.global_delete_min_size"
                  type="number"
                  min="0"
                  label="全局最低停止阈值（GB）"
                  clearable
                ...
                />
                <VTextField
                  v-model.number="settingsDraft.global_delete_max_size"
                  type="number"
                  min="0"
                  label="全局最高触发阈值（GB）"
                  clearable
              ...
              <div class="brushflow-overview-grid">
                <VSheet tag="section" class="brushflow-panel app-surface-static">
                  ...
                  <dl class="brushflow-facts">
                    ...
                    <div><dt>删种策略</dt><dd>{{ taskConfig.proxy_delete ? `动态 ${taskConfig.delete_min_size || '-'}-${taskConfig.delete_max_size || '-'} GB` : (taskConfig.delete_condition_mode === 'all' ? '满足全部条件' : '满足任一条件') }}</dd></div>
